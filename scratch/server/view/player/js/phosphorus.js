...
var P = (function () {
  ...
  IO.PROJECT_URL = 'http://localhost:8080/api/v1/project/';
  IO.ASSET_URL = 'http://localhost:8080/static/';
  IO.SOUNDBANK_URL = 'http://localhost:8080/static/';
  ...
  IO.decodeAudio = function (ab, cb) {
    if (audioContext) {
      IO.decodeADPCMAudio(ab, function (err, buffer) {
        if (buffer) return setTimeout(function () {
          cb(buffer)
        });
  ...
  IO.loadMD5 = function (md5, id, callback, isAudio) {
    ...
    if (ext === 'svg') {
      var cb = function (source) {
        ...
        var viewBox = svg.viewBox && svg.viewBox.baseVal ? svg.viewBox.baseVal : {
          width: 0,
          height: 0,
          x: 0,
          y: 0
  ...
  var Stage = function () {
    ...
    this.penContext = this.penCanvas.getContext('webgl', {
        preserveDrawingBuffer: true
      }) ||
      this.penCanvas.getContext('experimental-webgl', {
        preserveDrawingBuffer: true
      });
    ...
    this.glCollisionContext = this.glCollisionCanvas.getContext('webgl', {
        preserveDrawingBuffer: true
      }) ||
      this.glCollisionCanvas.getContext('experimental-webgl', {
        preserveDrawingBuffer: true
      });
  ...
  Stage.prototype.updateBackdrop = function () {
    ...
    */
    ...
    glDrawImage(
      ...
      (imgInfo.width / costume.resScale * costume.scale - 480) / 2, -(imgInfo.height / costume.resScale * costume.scale - 360) / 2,
  ...
  Sprite.prototype.draw = function (context, noEffects) {
    ...
    if (costume && costume.image.imgInfo) {
      ...
      glDrawImage(
        ...
        (imgInfo.height / costume.resScale / 2 - costume.rotationCenterY) * this.scale * costume.scale, [color, fisheye, whirl, pixelate, mosaic, brightness, ghost],
        this.stage.tColor);
    }
